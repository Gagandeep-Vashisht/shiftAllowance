...
const DashBoard = () => {
  ...
  const sorting = (col,type) => {
    ...
    if(type==="string"){
    if (order === "ASC") {
      const sorted = [...sortData].sort((a, b) => (a[col].toLowerCase() < b[col].toLowerCase() ? 1 : -1));
  ...
  if(type==="number"){
    if (order === "ASC") {
      const sorted = [...sortData].sort((a, b) => (parseInt(a[col]) < parseInt(b[col]) ? 1 : -1));
  ...
  const handleSaveClick = (event) => {
    ...
    event.preventDefault();
    const ngtShift = editData.nightShift === "" ? 0 : editData.nightShift;
    const aftShift =
  ...
  const handleFieldChange = (event) => {
    ...
    newFormData["transportAllowance"] =
      parseInt(newFormData["daysTa"]) * perDayFees;
    newFormData["totalAllowance"] =
      newFormData["transportAllowance"] +
      parseInt(newFormData["nightShift"] * 2 * perDayFees);
    ...
    <div>
      ...
      <div className="container ">
        <div className={classes.row}>
          ...
          <label></label>
          <div>
        <button className=" btn btn-primary" onClick={fetchDetail}>Filter</button></div>
        </div>
          </div>
